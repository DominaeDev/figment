...
//
template<typename T>
constexpr float round_v() { return std::is_integral_v<T> ? 0.5f : 0.0f; }

template<typename T>
constexpr T mul_flt(const T& value, float f) { return static_cast<T>(static_cast<float>(value) * f + round_v<T>()); }

template<typename T>
constexpr T mul_dbl(const T& value, double f) { return static_cast<T>(static_cast<double>(value) * f + round_v<T>()); }
    ...
    for(int i=0; i<h; i++) 
    {
        ...
        if constexpr(kernel == kLarge)
        {
            ...
            for(int ch=0; ch<C; ++ch)
            {
                acc[ch] += lv[ch] - fv[ch];
                // assert(acc[ch] >= 0);
                out[ti*C+ch] = mul_flt(acc[ch], iarr);
            }
        }
...
inline void horizontal_blur_kernel_crop(const T * in, T * out, const int w, const int h, const int r)
{
    ...
    for(int i=0; i<h; i++)
    {
        ...
        if constexpr(kernel == kLarge)
        {
            ...
            for(int ch=0; ch < C; ++ch)
            {
                out[j*C+ch] = mul_flt(acc[ch], iwidth);
            }
        }
        else if constexpr(kernel == kMid)
        {
            ...
            { 
                ...
                out[ti*C+ch] = mul_flt(acc[ch], inorm);
            }
            ...
            for(int ch=0; ch<C; ++ch)
            { 
                out[ti*C+ch] = mul_flt(acc[ch], iwidth);
            }
    ...
    for (int i = 0; i < h; i++)
    {
        ...
        if constexpr(kernel == kLarge) // generic but slow
        {
            ...
            { 
                ...
                out[ti*C+ch] = mul_dbl(acc[ch], iarr);
            }
        }
        else if constexpr(kernel == kMid)
        {
...
inline float sigma_to_box_radius(int boxes[], const float sigma, const int n)
{
    ...
    int wl = static_cast<int>(wi); // no need std::floor  
    if(wl%2==0) wl--;
    int wu = wl+2;
                
    float mi = (12*sigma*sigma - n*wl*wl - 4*n*wl - 3*n)/(-4*wl - 4);
    int m = static_cast<int>(mi+0.5f); // avoid std::round by adding 0.5f and cast to integer type
