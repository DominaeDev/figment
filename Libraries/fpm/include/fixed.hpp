...
// Added std::formatter boilerplate 
// - DominaeDev

template <typename BaseType, typename IntermediateType, unsigned int FractionBits>
struct std::formatter<fpm::fixed<BaseType, IntermediateType, FractionBits>>
    : std::formatter<double>
{
    auto format(const fpm::fixed<BaseType, IntermediateType, FractionBits>& value, std::format_context& context) const
    {
        return std::formatter<double>::format(static_cast<double>(value), context);
    }
};
#endif
