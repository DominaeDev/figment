export module LLMUtility;

import Common;
import LLMTypes;

extern "C++" class Context;

export 
{
	namespace llm_util
	{
		std::string stringFromToken(VocabPtr pVocab, Token token);
		size_t validate_utf8(const string& text) noexcept;
		size_t string_find_partial_stop(const std::string& str, const std::string& stop);
		size_t find_one_of(const string& text, const std::vector<string>& words);
		size_t find_stopping_strings(const string& text, const std::vector<string>& stop_words, const size_t last_token_size, bool is_full_stop);
		void get_tag_and_name(const string& text, string& tag, string& name);
		string& sanitize_response(string& text);
		string& complete_message(string& text);
		void process(string& partial, string str_token, bool& bWait, bool& bHalt, string& stop_word);
		std::pair<MessageType, bool> detect_message_type(string text) noexcept;

		Batch init_batch(int32_t ctx_size, int32_t n_seq_max);
		void free_batch(Batch& batch);

		bool init_embedding_batch(ModelPtr pModel, ContextPtr pCtx, const std::vector<Token>& tokens, Batch& out_pBatch);
		Batch create_batch(std::span<Token> tokens, std::span<LlamaSequenceId> seqs, int32_t n_seq_max, int32_t position);
		Batch create_batch_view(const Batch& batch, int32_t position, int32_t length);

		std::vector<Token> tokenize(VocabPtr pModel, string prompt, bool add_special = false);
		std::vector<Token> tokenize_and_batch(Context& context, string content, SequenceId seq_id, int32_t pos, bool add_special = false);
		std::optional<std::vector<Token>> tokenize_and_decode(Context& context, string content, SequenceId seq_id, int32_t pos, bool add_special = false);
		void erase_bottom(ContextPtr pCtx, int32_t n_max_seq, int32_t pos);

		std::string process_message(std::string message, std::string actorName, std::vector<Submessage>* out_pSubmessages = nullptr) noexcept;

		string format_id(string id);
		bool dump_batch_text(const Context& context, int32_t seq_id, string filename);
		bool dump_batch_tokens(const Context& context, int32_t seq_id, string filename);
		bool dump_kv_cache(const Context& context, int32_t seq_id, string filename);
		bool dump_kv_cache_cells(const Context& contextState, string filename);

		bool dump_kv_cache_cells(ContextPtr pCtx, int32_t num_sequences, string filename);
		bool dump_batch_tokens(const Batch& batch, int32_t num_tokens, int32_t seq_id, VocabPtr pVocab, string filename);

//		inline SequenceIndices get_sequence_indices(Sequence seq, int32_t n_seq_max) noexcept;
		SequenceIndices get_sequence_indices(SequenceId seq, int32_t n_seq_max) noexcept;
		SequenceId sequence_from_index(int32_t seq_idx) noexcept;
	}
	
	std::vector<string> const opening_tags = {
		std::format("<{0}=\"", Constants::Chat::DialogueTag),
		std::format("<{0}=\"", Constants::Chat::ActionTag),
		std::format("<{0}=\"", Constants::Chat::ThoughtTag),
		std::format("<{0}>", Constants::Chat::NarrationTag),
		std::format("<{0}>", Constants::Chat::DirectionTag),
	};

	std::vector<string> const closing_tags {
		std::format("</{0}>", Constants::Chat::DialogueTag),
		std::format("</{0}>", Constants::Chat::ActionTag),
		std::format("</{0}>", Constants::Chat::ThoughtTag),
		std::format("</{0}>", Constants::Chat::NarrationTag),
		std::format("</{0}>", Constants::Chat::DirectionTag),
	};
}

