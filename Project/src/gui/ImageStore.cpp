...
namespace fig::gui
{
	std::map<fig::uuid, fig::sdl::Surface> ImageStore::_images {};
	std::map<fig::uuid, fig::sdl::Texture> ImageStore::_textures {};
	std::map<fig::uuid, fig::uuid> ImageStore::_coverImages {};

	void ImageStore::Init()
	{
	}

	void ImageStore::Release()
	{
		_images.clear();
		_textures.clear();
		_coverImages.clear();
	...
	fig::uuid ImageStore::GetCoverImageID(const fig::uuid& characterAssetID) noexcept
	{
	...
	TexturePtr ImageStore::GetTexture(RendererPtr pRenderer, const fig::uuid& assetID)
	{
