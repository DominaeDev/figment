...
namespace fig::gui
{
	...
	void TopBar::Initialize(fig::string_view title)
	{
		...
		if (_pScrollPanel)
		{
			_pShadow = CreateControl<VerticalGradient>(Color::Border, Color::Border);
	...
	void TopBar::OnUpdate(float)
	{
		if (_pShadow and _pScrollPanel)
		{
			...
			if (fAlpha > 0.0f and GetBorderRenderer() == nullptr)
				SetBorderRenderer<LineBorderRenderer>(Color::Border, Direction::South);
			else if (fAlpha == 0.0f and GetBorderRenderer() != nullptr)
				SetBorderRenderer(nullptr);

			_pShadow->SetColors(fig::color_ref(Color::Border).WithAlpha(0.5f * fAlpha), fig::color_ref(Color::Border).WithAlpha(0.0f));
			_pShadow->SetVisible(fAlpha > 0.0f);
			if (fAlpha > 0.0f)
				GetBorderRenderer()->SetColor(fig::color_ref(Color::Border).WithAlpha(fAlpha));
		}
	}
}