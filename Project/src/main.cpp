...
int main(int argc, char *argv[])
{
    ...
    float ptsize;
    int i;
    SDL_Color white = { 0xFF, 0xFF, 0xFF, SDL_ALPHA_TRANSPARENT };
	...
	const char* fontFace = "fonts/Roboto-Medium.ttf";
    font = TTF_OpenFont(fontFace, ptsize);
    if (font == NULL) {
        SDL_Log("Couldn't load %g pt font from %s: %s",
                    ptsize, fontFace, SDL_GetError());
        result = 2;
        goto done;
    }
    ...
    TTF_SetFontHinting(font, (TTF_HintingFlags)hinting);
