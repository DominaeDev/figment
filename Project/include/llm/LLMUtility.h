...
namespace llm_util
{
	fig::string stringFromToken(VocabPtr pVocab, llama_token token);
	...
	std::vector<llama_token> tokenize(VocabPtr pModel, fig::string prompt, bool add_special = false);
	std::vector<llama_token> tokenize_and_batch(Context& context, fig::string content, SequenceId seq_id, int32_t pos, bool add_special = false);
	std::optional<std::vector<llama_token>> tokenize_and_decode(Context& context, fig::string content, SequenceId seq_id, int32_t pos, bool add_special = false);
